...
c gilles modif to avoid round off error bug
      if (abs(h-abs(tend-t)).le.1.d-12) then
         ...
         if (h.gt.abs(tend-t)) then
            write(6,*) 'initial step is longer than the integration',
     &           ' interval',h,abs(tend-t),h-abs(tend-t)
            arret=.true.
            idid=-1
            return
         end if
...
c -------- Call to the core integrator. -----------
     ...
     &     work,work(n1),work(n2),work(n3),work(n4),ms,atol,rtol,ntol,
     &     recf,fp1,fp2,iwork,arret,uround,idid,work(n5),recf2,recalph,
...
c ----------------------------------------------
     ...
     &     err,errp,tend,h,hnew,hp,facmax,fac,facp,rhodiff,rhoadv,
     &     uround,recalph(46),eigmax,eigmaxadv,t,te,ta,told,tmp,facd,
...
c
      ...
      if (iwork(23).eq.1)
     &     write (6,*) '--t',t,'h',h,'mdeg',mdeg+2,'ell',nell,'facd',
     &     facd
...
c avoid this computation if the step is rejected
     ...
     &           yjm1,yjm2,yjm3,yjm4,yjm5,yrk0,yn,ytmp,y2,yerrA,yerrR,
     &           fnc,errA,errR,errD2,atol,rtol,ntol,beta,frjac,ijac,
     &           iwork,idid,nell)
     ...
     &        errD,errD2,errA,errR,atol,rtol,ntol,frjac,ijac,iwork,idid,
     &        nell)
      end if
      if (iwork(23).eq.1)
...
c
     ...
     &     errA,errR,errD2,atol,rtol,ntol,beta,frjac,ijac,iwork,idid,
     &     nell)
c ----------------------------------------------
      ...
      do ix=1,neqn,npdes
         do k=1,50
            ...
            if (dojac) then
               do i=1,npdes
                  do j=1,npdes
                     frjac(im+(i-1)+(j-1)*npdes) =
     &                    -h*frjac(im+(i-1)+(j-1)*npdes)
                  end do
                  frjac(im+(i-1)*(npdes+1)) = frjac(im+(i-1)*(npdes+1))
     &                 + 1.d0
               end do
